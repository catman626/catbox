...
@tbe_register.register_operator("MselossCustom", trans_bool_to_s8=False)
...
def mseloss_custom(x_in__, y_in__, z_out_, kernel_name="mseloss_custom", impl_mode=""):
    ...
    origin_func_name = "mseloss_custom"
    ascendc_src_dir_ex = "mseloss_custom"
    ascendc_src_dir = "mseloss_custom"
    ascendc_src_file = "mseloss_custom.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator MselossCustom. kernel name is " + kernel_name)
    op_type = "MselossCustom"
...
def op_select_format(x_in__, y_in__, z_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, z_out_)
    result = check_op_cap("op_select_format", "MselossCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_in__, z_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, z_out_)
    result = check_op_cap("get_op_specific_info", "MselossCustom", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
